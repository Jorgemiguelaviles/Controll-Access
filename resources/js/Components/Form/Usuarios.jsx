...
const Usuarios = () => {
        ...
        const fetchData = async () => {
            try {
                const [resultSistemas, resultGroup, resultUser] = await Promise.all([
                    ...
                    getBackendData(`${domain}getUsuario?start=${start}&end=${end}`)
                ]);


                setmaxvalue(resultUser['maxvalue'])
    ...
    return (
        <div className="d-flex flex-column align-items-center w-75">
            <div className="mb-4">
                <Steppers3passos rota1={'/Sistemas'} rota2={'/Grupo'} rota3={'/Usuarios'} rota4={'/Aprovadores'} ativo1={false} ativo2={false} ativo3={true} ativo4={false} />
            </div>
            <div className="mb-4">
                <Filter valorMax={maxvalue} start={start} end={end} setStart={setstart} setEnd={setend} setLoading={setLoading} domain={domain} rotaBack={rota} subtitulo={'Filtrar usuários'} filter={filter} setFilter={setFilter} setDataFilter={setDataFilter} setDataFilterBool={setDataFilterBool} setMsg={setMsg} />
            </div>
            <div className="w-75">
