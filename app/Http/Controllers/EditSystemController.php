...
class EditSystemController extends Controller
{
    public function editsystemcontroller(Request $request)
    {
        ...
        try {
            ...
            $nomeDoSistemaOrigen = $request->input('nomeDoSistemaOrigen') ?? '';
            $tiposDeAcessoOrigen = $request->input('tiposDeAcessoOrigen') ?? '';

            $existingSystem = System::where('nome_do_sistema', $nomeDoSistema)
                ->where('nome_do_sistema', '!=', $nomeDoSistemaOrigen)
                ->first();
            ...
            $tiposDeAcessolist = explode(",", $tiposDeAcesso);
            $tiposDeAcessoOrigenlist = explode(",", $tiposDeAcessoOrigen);

            $diferencas = array_diff($tiposDeAcessolist, $tiposDeAcessoOrigenlist);

            $list1 = [];
            $list2 = [];

            if ($nomeDoSistema != $nomeDoSistemaOrigen || empty($diferencas)) {
                foreach ($tiposDeAcessolist as $index => $value) {
                    $tiposDeAcessoOrigenVerify = $tiposDeAcessoOrigenlist[$index];
                    $columnName = combineAndFormatStrings($nomeDoSistemaOrigen, $tiposDeAcessoOrigenVerify);
                    $newColumnName = combineAndFormatStrings($nomeDoSistema, $value);

                    if (Schema::hasColumn('users', $columnName)) {
                        Schema::table('users', function (Blueprint $table) use ($columnName, $newColumnName) {
                            $table->renameColumn($columnName, $newColumnName);
                        });
                    }
                    if (Schema::hasColumn('departaments', $columnName)) {
                        Schema::table('departaments', function (Blueprint $table) use ($columnName, $newColumnName) {
                            $table->renameColumn($columnName, $newColumnName);
                        });
                    }

                    $registros = Acesso::where('acesso', $columnName)->get();
                    foreach ($registros as $registro) {
                        $registro->acesso = $newColumnName;
                        $registro->save();
                    }
                }
            }
