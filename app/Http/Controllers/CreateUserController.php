...
class CreateUserController extends Controller
{
    public function createusercontroller(Request $request)
    {
        ...
        try {
            ...
            $all = $request->all('');
            $infosetor = $request->input('infoSetor');
            ...
            foreach ($listMain as $indice1 => $nome1) {
                foreach ($listSecondary as $indice2 => $nome2) {
                    ...
                    if ($database[1]['message'][1][$indice2]['acesso'] === $combinacao) {
                        $system->{$database[1]['message'][1][$indice2]['acesso']} = $systemStatusSecondary[$indice2 + 1];
                    }
                }
            }
